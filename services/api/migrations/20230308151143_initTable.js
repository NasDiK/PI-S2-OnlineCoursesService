...
exports.up = async (knex) => {
    await knex.schema.createTable('users',  (table) => {
        table.increments('id');
        table.string('nickname').comment('Логин пользователя').notNullable();
        table.string('avatar_url ').comment('Ссылка на аватарку');
        table.string('fullname').comment('Полное имя пользователя');
        table.string('password').comment('Пароль пользователя').notNullable();
    });
    await knex.schema.createTable('courses',  (table) => {
        table.increments('id');
        table.string('title').comment('Заголовок курса').notNullable();
        table.string('description').comment('Описание курса');
    });
    await knex.schema.createTable('groups',  (table) => {
        table.increments('id');
        table.string('title').comment('Заголовок группы').notNullable();
    ...
    await knex.schema.createTable('groups_users',  (table) => {
        ...
        table.boolean('isModerator').defaultTo(false).notNullable();
    });
    await knex.schema.createTable('roles',  (table) => {
        table.increments('id');
        table.string('title').comment('Заголовок роли').notNullable();
    ...
    await knex.schema.createTable('rights',  (table) => {
        ...
        table.integer('rights').notNullable().comment('Сумма прав');
    });
    await knex.schema.createTable('tasks',  (table) => {
        table.increments('id');
        table.integer('course_id');
        table.foreign('course_id').references('id').inTable('courses');
        table.string('title').comment('Заголовок задачи').notNullable();
    ...
    await knex.schema.createTable('tasks_logger',  (table) => {
        ...
        table.integer('action').comment('Решение задачи').notNullable();
    });
